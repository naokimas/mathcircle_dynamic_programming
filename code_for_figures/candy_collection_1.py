...
def make_house_polished(save_path="candy_collection_1.pdf", show=True):
    rewards = [2, -1, 4, 7]
    candy_counts = [2, 1, 4, 7] 
    ...
    for i in range(num_rooms):
        ...
        # 3. Room Labels (20% larger, 30% closer to edge)
        # Previous spacing was -0.6, 70% of that is -0.42
    ...
    # 7. Arrows (Tighter self-loops)
    for i in range(num_rooms):
        x_c = i * room_w + room_w/2
        
        # Self-loop (Stay)
        # Vertical gap reduced by 60%. Loop now sits very close to the reward text.
        loop_y = reward_y + 0.6 
