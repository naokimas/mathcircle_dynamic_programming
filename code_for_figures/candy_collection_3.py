...
def make_house_5_rooms(save_path="candy_collection_3.pdf", show=True):
    # Reward sequence: 1, -1, 4, 0, 6
    rewards = [1, -1, 4, 0, 6]
    # For candy counts, we use absolute value for display, 
    # but the bitter logic handles the -1 visually.
    candy_counts = [1, 1, 4, 0, 6] 
    
    num_rooms = len(rewards)
    room_w, room_h = 4.0, 3.5
    
    # Adjusted figure size for 5 rooms
    ...
    for i in range(num_rooms):
        ...
        for c in range(count):
            ...
            draw_candy(ax, cx, cy, s=0.22, bitter=is_bitter)
    ...
    for i in range(num_rooms):
        ...
        # Self-loop (Stay) - Tucked close to reward
        loop_y = reward_y + 0.6 
