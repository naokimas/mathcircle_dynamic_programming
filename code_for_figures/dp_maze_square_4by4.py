...
def draw_mouse(ax, x, y, s=0.35):
    """Authentic mouse drawing based on your provided style."""
...
def make_4x4_grid_maze(save_path="grid_maze_4x4.pdf", show=True):
    # Rewards matrix: 4x4 as specified
    rewards = np.array([[3, 1, 5, 2],
                        [4, 0, 2, 10],
                        [1, 3, 2, 1],
                        [2, 8, 2, 7]])
    ...
    # Start: Top-left node (0, (rows-1)*spacing)
    ax.text(0, (rows-1)*spacing + 0.8, "START", ha="center", va="bottom", 
            fontsize=14, fontweight="bold")
    
    # Goal: Bottom-right node ((cols-1)*spacing, 0)
    ax.text((cols-1)*spacing, -0.8, "GOAL", ha="center", va="top", 
            fontsize=14, fontweight="bold")

    # 4. Draw Mouse near Start
    # Positioned further left to ensure no overlap with the 'START' label
    mouse_x = -1.4
    mouse_y = (rows - 1) * spacing
    draw_mouse(ax, mouse_x, mouse_y, s=0.75)
    ...
    # Padding the limits
    ax.set_xlim(-3.0, (cols-1)*spacing + 2.0)
    ax.set_ylim(-1.5, (rows-1)*spacing + 2.0)
...
if __name__ == "__main__":
    make_4x4_grid_maze(save_path="dp_maze_square_4by4.pdf")
